...
function appDisplayValue(value){
...
function renderRowsWithTotal(tableId,headers,rows,limit,totalField='Material Description'){
 ...
 [...body,...(total?[total]:[])].forEach(r=>{let tr=document.createElement('tr');if(r===total)tr.className='total';headers.forEach(x=>{let td=document.createElement('td');td.textContent=appDisplayValue(r[x]??'');tr.appendChild(td)});b.appendChild(tr)});
}
